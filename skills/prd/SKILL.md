...
**核心要求：逐项与用户确认，每次只确认一个字段，不得一次性抛出全部内容让用户处理。**

按以下顺序逐项确认，每项给出推荐值并等待用户回应后再继续下一项：

1. **功能名称**：基于 PRD 正文内容自行提炼一个简洁准确的功能名称
2. **所属模块**：基于代码结构和功能分析，提炼该功能所属的模块名称
3. **作者**：默认取当前系统用户名（可通过 `whoami` 命令或读取 `USER` 环境变量获取）
4. **1.1 背景**：基于功能代码、项目名称、README、已有 SPEC 文件等上下文，推荐业务背景
5. **1.2 目标**：基于功能代码和项目上下文，推荐业务目标和量化指标
6. **1.3 Non-Goals**：基于功能范围和项目上下文，推荐明确不做的范围
7. **2. 用户故事**：基于代码中的用户角色、页面流程、权限控制等，推荐目标用户角色、故事描述、业务目的和优先级
